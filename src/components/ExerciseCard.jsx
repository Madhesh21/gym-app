...
export default function ExerciseCard(props) {
  ...
  return (
    <div className="bg-slate-950 flex flex-col gap-4 p-4 rounded-md sm:flex-wrap border border-solid border-blue-400">
      <div className="flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-x-4">
        <h4 className="text-3xl hidden sm:inline sm:text-4xl md:text-5xl font-semibold text-slate-400">
          0{i + 1}
        </h4>
        <h2 className="capitalize whitespace-nowrap truncate max-w-full flex-1 text-lg sm:text-xl md:text-2xl sm:text-center">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2">
        {["reps", "rest", "tempo"].map((info) => {
          return (
            <div
              key={info}
              className="flex flex-col p-2 w-full rounded border-[1.25px] border-solid border-slate-900"
            >
        ...
        <button
          ...
          className="flex flex-col p-2 w-full rounded border-[1.25px] border-solid border-blue-900 duration-200 hover:border-blue-600 "
        >
